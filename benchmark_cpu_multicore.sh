#!/bin/bash
# Benchmark script for CPU Multicore (OpenMP) implementation
# Usage: ./benchmark_cpu_multicore.sh [--runs N] [--output FILE] [--batch N]

# Default values
RUNS=10
OUTPUT="results/cpu_multicore.csv"
BATCH=1
MODEL_DIR="models"
...
echo "=== CPU Multicore (OpenMP) Benchmark ==="
echo "Runs: $RUNS"
...
for i in $(seq 1 $RUNS); do
    ...
    if command -v /usr/bin/time >/dev/null 2>&1; then
        ...
        TIME_OUTPUT=$(/usr/bin/time -v ./build/cnn_inference_cpu_multicore --model "$MODEL_DIR" --device cpu_multicore 2>&1)
        ...
        ./build/cnn_inference_cpu_multicore --model "$MODEL_DIR" --device cpu_multicore > /tmp/inference_output_$$.txt 2>&1 &
        INFERENCE_PID=$!
