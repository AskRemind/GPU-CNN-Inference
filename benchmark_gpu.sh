#!/bin/bash
# Benchmark script for GPU CUDA implementation
# Usage: ./benchmark_gpu.sh [--runs N] [--output FILE] [--batch N]
...
# Write CSV header
echo "run,batch_size,time_ms,memory_mb,gpu_memory_mb" > "$OUTPUT"
...
for i in $(seq 1 $RUNS); do
    ...
    if command -v /usr/bin/time >/dev/null 2>&1; then
        ...
        TIME_OUTPUT=$(/usr/bin/time -v ./build/cnn_inference_gpu --model "$MODEL_DIR" --device gpu 2>&1)
        ...
        ./build/cnn_inference_gpu --model "$MODEL_DIR" --device gpu > /tmp/inference_output_$$.txt 2>&1 &
        INFERENCE_PID=$!
    ...
    # Write to CSV
    echo "$i,$BATCH,$TIME_MS,$MEMORY_MB,$GPU_MEMORY_MB" >> "$OUTPUT"
    
    # Print progress
    echo "  Time: $TIME_MS ms, Memory: $MEMORY_MB MB, GPU Memory: $GPU_MEMORY_MB MB"
done
